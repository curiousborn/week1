...
template sudoku() {
    ...
    //[assignment] hint: you will need to initialize your RangeProof components here
	component rangeProofPuzzle[81];
    component rangeProofSolution[81];  
    var x = 0;    
    //component rangeProof1 = RangeProof(32);
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) { 
			...
			//assert(puzzle[i][j]>=0); //[assignment] change assert() to use your created RangeProof instead
			// Initialize template
            rangeProofPuzzle[x] = RangeProof(32);
			//	Assign input
			rangeProofPuzzle[x].in <== puzzle[i][j];
			// Assign the range
			rangeProofPuzzle[x].range[0] <== 0;
            rangeProofPuzzle[x].range[1] <== 9;
			// Assert output is 1
            1 === rangeProofPuzzle[x].out;
			// Initialize template
            rangeProofSolution[x] = RangeProof(32);           
			//	Assign input
			rangeProofSolution[x].in <== solution[i][j];
			// Assign the range
			rangeProofSolution[x].range[0] <== 0;
            rangeProofSolution[x].range[1] <== 9;
			// Assert output is 1
            1 === rangeProofSolution[x].out;
